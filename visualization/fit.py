...
def subsample(num_frames, last_framerate, new_framerate):
    ...
    if step < 1:
        raise ValueError("new_framerate cannot exceed last_framerate")
...
faces_path = Path(config.SMPL_MODEL_DIR) / "smplh.faces"
if not faces_path.exists():
    with faces_path.open("wb") as handle:
        np.save(handle, smplmodel.faces)
